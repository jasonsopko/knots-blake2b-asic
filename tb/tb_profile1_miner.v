...
module tb_profile1_miner;
    ...
    initial begin
        ...
        $display("  searched %0d nonces in %0d cycles, %0d per hash, %0d hit(s)",
                 hashes, cycles, cycles / hashes, hits);
        if (hits != 1) begin
        ...
        if (fails == 0)
            $display("profile1_miner: found the nonce the model found");
        else
            $display("profile1_miner: %0d FAILURES", fails);
        $finish;
    end

    initial begin
        if ($test$plusargs("vcd")) begin
            $dumpfile("build/tb_miner.vcd");
            $dumpvars(0, tb_profile1_miner);
        end
    end

    initial begin
        #2000000;
        $display("profile1_miner: TIMEOUT");
        $finish;
    end
